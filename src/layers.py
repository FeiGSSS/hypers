...
#         self.lin_O = Linear(heads*self.hidden, self.hidden) # For heads combining
        ...
        self.rFF = MLP2(in_channels=self.heads*self.hidden,
...
#         This is because O = softmax(QK^T)V and V = V_in*W^V. So W^O can be effectively taken care by W^V!!!
    ...
    def message(self, x_j, alpha_j,
        ...
        alpha = softmax(alpha, index, ptr, index.max()+1)  # index: node-edge
        self._alpha = alpha
        alpha = F.dropout(alpha, p=self.dropout, training=self.training)
        return x_j * alpha.unsqueeze(-1)

    def aggregate(self, inputs, index,
                  dim_size=None, aggr='mean'):  #  aggr=None
...
class HalfNLHconv(MessagePassing):
    def __init__(self,
        ...
        if self.attention:
            ...
            if num_layers > 0:
                self.f_enc = MLP2(in_dim, hid_dim, hid_dim, num_layers, dropout, Normalization, InputNorm)
                self.f_dec = MLP2(hid_dim, hid_dim, out_dim, num_layers, dropout, Normalization, InputNorm)
            else:
